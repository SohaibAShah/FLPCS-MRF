...
def loadClientsData():
    ...
    clint_index = 0
    for sub_ in subs:
        if sub_ in subsToActivitylLost:
            SUB_train = pd.read_csv('./dataset/Sensor + Image/{}_sensor_activity7-11lost_train.csv'.format(sub_), skiprows=1)
        elif sub_ in subsToTrialLost:
            SUB_train = pd.read_csv('./dataset/Sensor + Image/{}_sensor_trial2Lost_train.csv'.format(sub_),
                                    skiprows=1)
        else:
        ...
        cam = '1'
        if sub_ in subsToActivitylLost:
            image_train = './dataset/Sensor + Image' + '/' + '{}_image_1_activity7-11lost_train.npy'.format(sub_)
            name_train = './dataset/Sensor + Image' + '/' + '{}_name_1_activity7-11lost_train.npy'.format(sub_)
            label_train = './dataset/Sensor + Image' + '/' + '{}_label_1_activity7-11lost_train.npy'.format(sub_)
        elif sub_ in subsToTrialLost:
            image_train = './dataset/Sensor + Image' + '/' + '{}_image_1_trial2Lost_train.npy'.format(sub_)
            name_train = './dataset/Sensor + Image' + '/' + '{}_name_1_trial2Lost_train.npy'.format(sub_)
            label_train = './dataset/Sensor + Image' + '/' + '{}_label_1_trial2Lost_train.npy'.format(sub_)
        else:
        ...
        cam = '2'
        if sub_ in subsToActivitylLost:
            image_train = './dataset/Sensor + Image' + '/' + '{}_image_2_activity7-11lost_train.npy'.format(sub_)
            name_train = './dataset/Sensor + Image' + '/' + '{}_name_2_activity7-11lost_train.npy'.format(sub_)
            label_train = './dataset/Sensor + Image' + '/' + '{}_label_2_activity7-11lost_train.npy'.format(sub_)
        elif sub_ in subsToTrialLost:
            image_train = './dataset/Sensor + Image' + '/' + '{}_image_2_trial2Lost_train.npy'.format(sub_)
            name_train = './dataset/Sensor + Image' + '/' + '{}_name_2_trial2Lost_train.npy'.format(sub_)
            label_train = './dataset/Sensor + Image' + '/' + '{}_label_2_trial2Lost_train.npy'.format(sub_)
        else:
...
def pareto_optimization(
    ...
    pareto_scores = [0.6 *  rf_acc_global[i] - 0.4 * rf_loss[i]  for i in range(len(rf_loss))]
...
def get_top_clients_with5RF(rf_loss, rf_acc_train, rf_acc_val, rf_acc_global, p_loss, p_bias, client_num):
    ...
    top_client_ids = np.argsort(scores)[::-1][:client_num].tolist()  # 降序排序，取前 clientNum 个
    lst = [x - 1 for x in top_client_ids]
    return lst,origin_scores
...
def trainValModelCSVIMG(model_name, svmethod, total_client,num_clients,epoch,max_acc,epoch_size,local_epoch_per_round,round_early_stop,
    ...
    clients = []
    for client_index in range(total_client-1):
        ...
        if epoch_count < round_early_stop+1:
            ...
            for client_index in range(total_client-1):
                ...
                test_server_loader = torch.utils.data.DataLoader(
                    CostumDataset(X_train_csv_scaled_splits[total_client-1], X_train_1_scaled_splits[total_client-1],
                                  X_train_2_scaled_splits[total_client-1], Y_train_csv_splits[total_client-1]),
            ...
            if svmethod == '5RF':
                candidates,scores = get_top_clients_with5RF(rf_relative_loss_reduction, rf_acc_train,
                ...
                candidates = pareto_optimization(rf_relative_loss_reduction, rf_acc_train,
                                                 rf_global_validation_accuracy, rf_loss_outliers, rf_performance_bias,
                                                 num_clients)
            elif svmethod =='random':
                candidates = np.random.choice(total_client-1, num_clients, replace=False)
            ...
            dataset_size = 0
            test_server_loader = torch.utils.data.DataLoader(
                CostumDataset(X_test_csv_scaled_splits[total_client-1], X_test_1_scaled_splits[total_client-1],
                              X_test_2_scaled_splits[total_client-1], Y_test_csv_splits[total_client-1]),
                batch_size=epoch_size)
...
# subs = [1, 3, 4, 7, 10, 11, 12, 13, 14, 15, 16, 17]
# subsToTrialLost = [13, 14, 15, 16, 17]
subsToActivitylLost = [13, 14, 15, 16, 17]
# subsToActivitylLost = []
subsToTrialLost = []
...
def main():
    ...
    for model_name in model_names:
        if model_name =='tc1c2ResModelV3DataV4AdamWithSCVLost':
            # Node csv data replaced
            for index in [6, 7, 8, 9, 10]:
                ...
                X_train_csv_scaled_splits[index] = placed_train
                X_test_csv_scaled_splits[index] = placed_test
        elif model_name == 'tc1c2ResModelV3DataV4AdamWithImgLost':
