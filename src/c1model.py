...
def trainValImg1Model(total_client,num_clients,epoch,max_acc,
    ...
    test_loader = torch.utils.data.DataLoader(
        CostumDataset(X_val_1_scaled_splits[total_client - 1], Y_val_1_splits[total_client - 1]), batch_size=32)
...
def main():
    ...
    trainValImg1Model(total_client, num_clients, epoch, max_acc,
                      X_train_1_scaled_splits, X_test_1_scaled_splits, X_val_1_scaled_splits,
                      Y_train_1_splits, Y_test_1_splits, Y_val_1_splits)
...
class Server(object):
    def __init__(self, model, eval_dataset, num_clients):
        ...
        self.serverTestDataSet = CostumDataset(eval_dataset[0],eval_dataset[1])
...
class Client(object):
    def __init__(self, model, train_dataset,val_dataset, id = -1):
                ...
                self.client_id = id
                self.train_dataset = CostumDataset(train_dataset[0],train_dataset[1])
                self.train_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size=32)
                self.eval_dataset = CostumDataset(val_dataset[0], val_dataset[1])
