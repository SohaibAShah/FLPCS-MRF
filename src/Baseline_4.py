...
def load_clients_data():
    data = {}
    print("Loading data from ./dataset/Sensor + Image/ ...")
...
def train_client_fedavg_prox(global_model, client_data, use_prox=False, mu=0.0):
    """Generic training function for FedAvg and FedProx"""
    ...
    # Store global parameters for FedProx calculation
...
def run_pareto_fl(clients_data, initial_model):
    ...
    for round_num in range(1, ROUNDS + 1):
        ...
        # Train ALL clients locally to gather metrics for selection
        for cid in range(len(clients_data)):
            ...
            # Simulated Global Acc (Expensive but required for Pareto logic provided)
