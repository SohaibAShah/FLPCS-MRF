...
class CustomDataset(Dataset):
    def __init__(self, csv_features, img1_features, img2_features, labels):
        ...
        # FIX 1: Ensure labels are a flat 1D tensor to prevent shape mismatch errors
...
def pareto_optimization(metrics, num_clients):
    data = np.array(metrics).T  # Shape: (6, num_clients)
    def is_dominated(i):
...
if __name__ == "__main__":
    ...
    # Assume all clients have same CSV structure, take dim from first client
    csv_dim = clients_data[0]['train'][0].shape[1]
    global_model = ModelCSVIMG(csv_dim).to(device)

    print("=== Federated Learning with Pareto Client Selection (Full Multimodal) ===")
    best_acc = 0.0
    ...
    for round_num in range(1, ROUNDS + 1):
        ...
        client_metrics = []
        # FIX 2: Store updates separately for each client
        all_client_updates = {} 
        ...
        for cid in range(TOTAL_CLIENTS):
            ...
            # Global test accuracy (on all clients' test data) to calculate bias
            # Note: In real FL, this is expensive/impossible, but used here for simulation
            ...
            # FIX 2 (cont): Store the update for this specific client
            local_update = {}
        ...
        # FIX 3: Aggregate only from selected clients
        global_update = {name: torch.zeros_like(param) for name, param in global_model.state_dict().items()}

        for name in global_update:
            # Stack the updates from the selected clients
            updates_stack = torch.stack([all_client_updates[cid][name] for cid in selected_clients])
            
            # Check if the tensor is a floating point type (weights/biases) or integer (num_batches_tracked)
            if updates_stack.is_floating_point():
                global_update[name] = updates_stack.mean(dim=0)
            else:
                # Handle integer buffers (like BatchNorm num_batches_tracked)
                # Cast to float -> calculate mean -> cast back to original dtype
        ...
        # Global evaluation (Final check for the round)
        global_model.eval()
        correct, total = 0, 0
        with torch.no_grad():
            for cid in range(TOTAL_CLIENTS):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(global_model.state_dict(), "best_pareto_multimodal.pth")
